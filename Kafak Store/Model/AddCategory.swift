...
struct Api_AddCategory : Codable {
    
    let result : Res_AddCategory?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent(Res_AddCategory.self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct Res_AddCategory : Codable {
    let id : String?
    let store_id : String?
